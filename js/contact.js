...
const contactValidationContainer = document.querySelector(".contact-validation");
const modalOverlay = document.querySelector(".modal-overlay");
const validationCloseButton = document.querySelector(".icon-close");

form.addEventListener("submit", validateForm);

modalOverlay.addEventListener("click", () => closeModal(contactValidationContainer, modalOverlay));
validationCloseButton.addEventListener("click", () => closeModal(contactValidationContainer, modalOverlay));
...
function validateForm(event) {
    ...
    if (validateLength(name, name.value, 5, nameError) && validateEmail(email, email.value, emailError) && validateLength(subject, subject.value, 15, subjectError) && validateLength(message, message.value, 25, messageError)) {
        form.reset();
        openModal(contactValidationContainer, modalOverlay);
    }
};
